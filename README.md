...
| `matches.origins` | array of strings | one of cookies/origins | Exact hosts (`maps.googleapis.com`) or wildcard form `*.suffix` (matches both the apex `suffix` and every subdomain `*.suffix`). Either form may carry a trailing path prefix (`www.google.com/maps/`) for a host that serves two products needing different consent treatment — a path-scoped entry is more specific and resolves before a bare host claim. Consumers that know only a host match such an entry on its host part alone. |
